-- Model v1.2 - v1.1 with 500k records from year to date
-- This uses the same features as v1.1 but with a much larger dataset (like v3)

-- Create all necessary UDFs for feature extraction (same as v1.1)
...
-- Create or replace the model v1.2
CREATE OR REPLACE MODEL `helix-225321.pii_detection.url_segment_classifier_v1_2`
OPTIONS(
...
-- Training data with query string handling and larger dataset (like v3)
WITH real_segments AS (
  ...
  LIMIT 500000  -- Same size as v3
),

synthetic_pii AS (
  -- Generate 500,000 synthetic PII samples (same as v3)
  SELECT 
    CASE 
