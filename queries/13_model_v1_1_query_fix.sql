-- Model v1.1 - Original model with query string fix
-- This applies the query string handling improvement to the v1 model

-- Create all necessary UDFs for feature extraction (same as v1)
...
-- Generate synthetic PII functions (same as v1)
...
-- Create or replace the model v1.1
CREATE OR REPLACE MODEL `helix-225321.pii_detection.url_segment_classifier_v1_1`
OPTIONS(
...
-- Training data with query string handling (same dataset size as v1)
WITH real_segments AS (
  ...
  FROM (
    WITH urls AS (
      ...
      WHERE time > '2025-05-22'
        AND time < '2025-05-23'  -- Same 1-day range as v1
    ),
    segments_array AS (
      SELECT DISTINCT
        hostname,
  ...
  LIMIT 50000  -- Same size as v1
),

synthetic_pii AS (
  -- Generate 50,000 synthetic PII samples (same as v1)
  SELECT 
    CASE 
...
SELECT
  ...
  -- All numeric features for the model (same as v1)
  length,
  entropy,
  ioc,
  letter_ratio,
