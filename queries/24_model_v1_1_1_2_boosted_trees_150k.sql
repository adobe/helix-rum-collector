-- Model v1.1.1.2 - Boosted Trees with 150k balanced dataset
...
-- Create or replace the model v1.1.1.2 with Boosted Trees and 150k dataset
CREATE OR REPLACE MODEL `helix-225321.pii_detection.url_segment_classifier_v1_1_1_2`
OPTIONS(
...
-- Training data with balanced synthetic generation (150k target)
WITH real_segments_raw AS (
  -- Extract real URL segments from a 3-day window for better temporal consistency
  SELECT DISTINCT
  ...
  FROM (
    WITH urls AS (
      ...
      WHERE time >= '2025-05-20'
        AND time < '2025-05-23'  -- 3-day window
    ),
    segments_array AS (
      SELECT DISTINCT
        hostname,
  ...
  LIMIT 150000  -- Target 150k real segments
),
