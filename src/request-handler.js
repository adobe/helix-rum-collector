...
function respondError(message, status, e, cxErrorLogger) {
  ...
  console.error(msg);
  try {
    cxErrorLogger.logError(status, message);
  } catch (err) {
...
export async function handleRequest(req, coralogixLogger, coralogixErrorLogger, goggleLogger) {
  try {
    ...
    if (!id) {
      return respondError('id field is required', 400, undefined, coralogixErrorLogger);
    }
    if (!weight || typeof weight !== 'number') {
      return respondError('weight must be a number', 400, undefined, coralogixErrorLogger);
    }
    if (typeof cwv !== 'object') {
      return respondError('cwv must be an object', 400, undefined, coralogixErrorLogger);
    }

    try {
      coralogixLogger.logRUM(
        cwv,
        id,
        weight,
        referer || referrer,
